...
TEXT;
     ...
     * @return array 发送结果
     */
    public function sendResetPasswordEmail($to, $username, $resetUrl) {
        $subject = '重置密码 - ' . MAIL_FROM_NAME;
        $body = $this->generateResetPasswordEmailHtml($username, $resetUrl);
        $altBody = $this->generateResetPasswordEmailText($username, $resetUrl);
     ...
     * 生成重置密码邮件 HTML 内容
     */
    private function generateResetPasswordEmailHtml($username, $resetUrl) {
        $siteName = MAIL_FROM_NAME;
        $expiryMinutes = defined('RESET_TOKEN_EXPIRY') ? (int)ceil(RESET_TOKEN_EXPIRY / 60) : 30;
        
        // 读取邮件模板
        $templatePath = __DIR__ . '/email_templates/reset_password.html';
     ...
     * 生成重置密码邮件纯文本内容
     */
    private function generateResetPasswordEmailText($username, $resetUrl) {
        $siteName = MAIL_FROM_NAME;
        $expiryMinutes = defined('RESET_TOKEN_EXPIRY') ? (int)ceil(RESET_TOKEN_EXPIRY / 60) : 30;
...
我们收到了您重置 {$siteName} 账户密码的请求。

请点击以下链接设置新密码：
{$resetUrl}

此链接将在 {$expiryMinutes} 分钟后过期。
重置成功后，所有已登录设备将需要重新登录。

如果您没有请求重置密码，请忽略此邮件，您的密码不会发生任何改变。
