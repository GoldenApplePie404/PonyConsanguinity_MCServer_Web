...
class AuthHelper
{
    ...
    /**
     * 删除某用户的所有会话（重置密码后强制下线）
     *
     * @param string $username 用户名
     * @return int 删除的会话数量
     */
    public static function destroyAllSessionsByUsername(string $username): int
    {
        $sessions = secureReadData(SESSIONS_FILE);
        $removed = 0;
    ...
    /**
     * 删除某用户的会话，但保留指定 token（修改密码后踢其他设备）
     *
     * @param string $username  用户名
     * @param string $keepToken 要保留的会话 token
     * @return int 删除的会话数量
     */
    public static function destroyOtherSessionsByUsername(string $username, string $keepToken): int
    {
        $sessions = secureReadData(SESSIONS_FILE);
        $removed = 0;
