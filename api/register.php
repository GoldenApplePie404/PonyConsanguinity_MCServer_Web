...
// 检查邮箱是否已被使用（邮箱唯一，重置密码依赖邮箱定位账号）
foreach ($users as $otherUsername => $otherUser) {
    if ($otherUsername !== $username && isset($otherUser['email']) && strtolower($otherUser['email']) === strtolower($email)) {
        json_response(false, '该邮箱已被注册，请更换邮箱或找回密码', null, 400);
    }
}
