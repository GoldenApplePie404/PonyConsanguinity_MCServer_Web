...
class SkillEngine {
     ...
     * 2026-08-21：迁移至新 mcstatus API（参数 ?host=&port=，响应 success/code/message/data）
     */
    private function queryExternalAPI($debugInfo = []) {
        $apiUrl = rtrim(MCSTATUS_API_URL, '/') . '/ping?host=' . urlencode($this->mcServerHost) . '&port=' . (int)$this->mcServerPort;
        ...
        if ($httpCode === 200 && $response) {
            ...
            // 新 API 返回格式：{"success": true, "code": 0, "message": "成功", "data": {...}}
            if ($data && ($data['success'] ?? false) === true && ($data['code'] ?? -1) === 0 && isset($data['data'])) {
                ...
                return [
                    ...
                    'players' => [
                        ...
                        'list' => $serverData['players']['sample'] ?? []
                    ],
                    'version' => [
                        'name' => $serverData['version']['name'] ?? 'Unknown',
                        'protocol' => $serverData['version']['protocol'] ?? 0
                    ],
                    'motd' => $serverData['motd']['plain_text'] ?? '',
                    'hostname' => $this->mcServerHost,
                    'port' => $this->mcServerPort,
                    'debug' => $debugInfo
                ];
