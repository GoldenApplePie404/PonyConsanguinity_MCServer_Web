...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($action === 'send') {
    ...
    } elseif ($action === 'admin_delete_conversation') {
        // 管理员删除整个会话（含全部消息）
        $user = AuthHelper::getCurrentUser();
        if (!$user || ($user['role'] ?? 'user') !== 'admin') json_response(false, '权限不足');
        $convId = $input['conv_id'] ?? '';
        if (!$convId) json_response(false, '缺少会话ID');
        $data = read_json(MESSAGES_FILE);
        if (!isset($data['conversations'][$convId])) json_response(false, '会话不存在');
        unset($data['conversations'][$convId]);
        unset($data['messages'][$convId]);
        write_json(MESSAGES_FILE, $data);
        json_response(true, '会话已删除');

    } elseif ($action === 'admin_rename_conversation') {
        // 管理员重命名会话（仅群聊）
        $user = AuthHelper::getCurrentUser();
        if (!$user || ($user['role'] ?? 'user') !== 'admin') json_response(false, '权限不足');
        $convId = $input['conv_id'] ?? '';
        $name = trim($input['name'] ?? '');
        if (!$convId) json_response(false, '缺少会话ID');
        if ($name === '') json_response(false, '名称不能为空');
        $data = read_json(MESSAGES_FILE);
        if (!isset($data['conversations'][$convId])) json_response(false, '会话不存在');
        if (($data['conversations'][$convId]['type'] ?? '') !== 'group') {
            json_response(false, '仅群聊可重命名');
        }
        $data['conversations'][$convId]['name'] = $name;
        write_json(MESSAGES_FILE, $data);
        json_response(true, '已重命名为：' . $name);
