<?php
// ℹ️ 外部契约入口（2026-08-17 标注）：本端点为定时任务入口，由外部调度器（crontab/计划任务）按 AFDIAN_CRON_* 配置调用，
// 项目内部代码不引用属正常现象，请勿删除。⚠️ 若线上尚未配置定时任务则当前实际未生效（等同闲置）。
/**
 * 爱发电订单定时更新脚本
 */
...
// 关闭页面错误显示，避免 Web 访问时泄露服务器路径；错误仍走日志模块记录
