<?php
// ℹ️ 外部契约入口（2026-08-17 标注）：本端点由「爱发电平台」服务器回调调用（在爱发电开发者后台配置 webhook 地址），
// 项目内部代码不引用属正常现象，请勿删除。
/**
 * 爱发电Webhook接收接口
 * 用于接收爱发电的订单推送通知
...
try {
    ...
    if ($orderUpdateMode === 'webhook' || $orderUpdateMode === 'all') {
        ...
        if ($config['webhook']['verify_sign']) {
            ...
            // openssl_verify 返回 1=通过 / 0=失败 / -1=异常。
            // 不能用 !verifySign()，因为 -1（异常）经 ! 会变成 false 而被误判为"通过"。
            if (verifySign($signStr, $receivedSign) !== 1) {
        ...
        if ($data['data']['type'] == 'order' && $orderStatus == 2) {
            ...
            // 仅处理黄金券方案；其它方案（VIP月卡/年卡当前未正式接入）忽略，
            // 避免生成 points_added=0 的"幽灵已完成"记录
            $goldenTicketPlanId = isset($config['plan_ids']['golden_ticket']) ? $config['plan_ids']['golden_ticket'] : '';
            if ($order['plan_id'] !== $goldenTicketPlanId) {
                if (function_exists('log_info')) {
                    log_info("非黄金券订单，忽略: {$outTradeNo}, plan_id={$order['plan_id']}", 'aifadian_webhook');
                } else {
                    logToFile("非黄金券订单，忽略: {$outTradeNo}, plan_id={$order['plan_id']}", 'info');
                }
