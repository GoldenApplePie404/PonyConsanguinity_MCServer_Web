...
// 共享密钥鉴权（防滥用）：保护所有 aifadian API 端点
afdian_require_auth();
...
function handleGetStatistics($config) {
    ...
    try {
        ...
        // 表无 amount 列，金额由点数反推（1元=100点），与客服工具 query_my_orders 口径一致
        $result = $db->fetchOne("SELECT SUM(points_added) as total_points, SUM(points_added)/100 as total_amount FROM afdian_orders WHERE status = 'completed'");
        if ($result) {
