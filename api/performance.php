...
// 获取服务器状态数据
try {
    // 与 mcstatus.php 同源：调用新 mcstatus API（参数 ?host=&port=，2026-08-21 起旧 ?ip= 参数已废弃）
    require_once 'config.php';
    
    // 获取外部 API 响应
    $api_url = rtrim(MCSTATUS_API_URL, '/') . '/ping?host=' . urlencode(MC_SERVER_IP) . '&port=' . (int)MC_SERVER_PORT;
    ...
    if ($response) {
        ...
        // 新 API 成功判定：success=true 且 code=0；data.players.online 为在线人数
        if ($data && ($data['success'] ?? false) === true && ($data['code'] ?? -1) === 0) {
