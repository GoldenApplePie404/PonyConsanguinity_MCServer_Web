...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Version-5.7-blue?style=for-the-badge" alt="Version">
...
- **服务器介绍**：详细的服务器信息和玩法介绍（你说的对，但是万驹同源是由EYPA团队……后面的想不起来了）
...
- **论坛系统**：支持 Markdown 格式的帖子发布和回复，具备分类功能，支持 LaTeX 公式与 Mermaid 图表，代码块语法高亮（支持行号显示和折叠展开）
...
- **pages/skin.html** - 皮肤站页面，自定义玩家皮肤和披风，不过暂时没法内嵌皮肤站，因此仅作为提示跳转页。若想搭建皮肤站，可以采用[Blessing Skin Server](https://github.com/bs-community/blessing-skin-server)
...
- **pages/post-detail.html** - 帖子详情页，查看帖子内容和回复，基于markdown文件解析生成，支持LaTeX公式与Mermaid图表，支持图片上传（得保证你有足够多的存储空间）
- **pages/announcement.html** - 公告列表页，查看服务器公告
- **pages/announcement-detail.html** - 公告详情页，查看公告详细内容，基于markdown文件解析生成
- **pages/shop.html** - 积分商城页面，用户可在商城中兑换积分商品
...
- **pages/complete_profile.html** - 完善个人信息页，与第三方登录有关，当用户使用的是EYPA登录，则先会跳转至此页面
- **pages/eypa_login_success.html** - EYPA登录成功页，登录成功后跳转至此页面

#### 管理功能页面

- **pages/logs.html** - 开发日志页，查看万驹同源服务器的开发历程
- **pages/rules.html** - 服务器规则页，查看游戏规则
- **pages/payment.html** - 充值中心页，黄金券充值功能，基于爱发电支付集成
- **pages/verify.html** - 邮箱验证结果页，显示验证成功/失败状态
- **tool/announcement-manager.html** - 公告管理工具，管理员可发布和管理公告，支持动态加载和编辑
- **tool/auto_update.html** - 自动更新账单工具，管理员可利用此页面手动更新充值账单
- **tool/gallery-management.html** - 图库管理工具，管理员可上传、删除、管理服务器上的图片
- **tool/user-management.html** - 用户管理工具，管理员可查看所有用户信息（用户名、ID、邮箱、创建时间、角色、邮箱验证状态、积分）
...
- **pages/upload.html** - 帖子图片上传测试页面，暂时不纳入网站，仅供测试使用
...
-   **本地开发**：任意静态文件服务器
-   **生产部署**：支持 PHP 的 Web 服务器（如 Apache、Nginx），建议使用PHP 8.0+，且你需要在php.ini中开启mysqli、openssl、pdo_mysql扩展，可以参考我的`config/php.ini`
...
2.  **启动本地服务器**
    -   使用 PHP 内置服务器
        ...
        ```
    -   或使用其他静态文件服务器（如 VS Code Live Server 扩展，建议不要使用live server，因为它会导致一些问题，推荐使用php或者python的http.server）
...
5.  **部署环境测试**
    ...
    如果你想在部署环境中进行测试（因为我们有些功能只能在生产环境中运行），可以尝试使用[ngrok](https://ngrok.com/)等工具将本地服务器暴露到公网，然后访问公网地址进行测试。
...
-   目前仅支持 EYPA 第三方登录
-   注册时可以 EYPA 账号，后续登录可以直接使用 EYPA 账号登录
...
基于 DeepSeek AI 的 Minecraft 服务器智能客服系统，支持 RAG（检索增强生成）、技能引擎、意图识别等高级功能。

### 核心功能

#### 1. AI 智能对话
- **DeepSeek AI 驱动**：使用 DeepSeek API 进行自然语言处理
- **RAG 系统**：检索增强生成，基于知识库精准回答
- **Markdown 渲染**：支持富文本格式回复
...
#### 3. 双层意图识别
- **AI 意图识别（优先）**：使用 AI 模型进行语义理解
- **规则匹配（后备）**：基于正则表达式的规则匹配
- **置信度评估**：自动评估识别结果可靠性

### 访问方式

- **个人中心页面**：访问 `pages/profile.html`，点击右下角 AI 客服按钮
- **直接访问**：`/pages/ai/kefu.html`
...
return [
    'deepseek' => [
        'api_key' => 'your_api_key_here',      // DeepSeek API 密钥
    ...
    'mc_server' => [
        'host' => 'mcpc.goldenapplepie.xyz',   // 服务器地址
        'port' => 25565                        // 服务器端口
    ]
];
```
...
| 参数 | 说明 | 推荐值 |
|------|------|--------|
| `api_key` | DeepSeek API 密钥，用于身份验证 | 从 DeepSeek 官网获取 |
| `model` | AI 模型名称 | `deepseek-chat` |
| `temperature` | 控制回复创造性（0=精确，1=有创意） | `0.7`（平衡） |
| `max_tokens` | 单次回复最大 Token 数 | `2000` |
| `host` | Minecraft 服务器地址 | 你的服务器域名 |
| `port` | Minecraft 服务器端口 | `25565`（默认） |

#### 3. 知识库维护

知识库采用 Markdown 格式，按主题分块，存储在 `api/ai/knowledge_base.md` 文件中。
...
| 技能名称 | 功能 | 触发示例 |
|---------|------|---------|
| `server_status` | 查询服务器状态 | "服务器状态如何？" |
| `player_count` | 查询玩家数量 | "多少人在线？" |
| `player_query` | 查询特定玩家 | "查询玩家 Steve" |
| `server_version` | 查询服务器版本 | "服务器版本是什么？" |

#### 技能执行流程

```
用户提问 → 意图识别 → 执行技能 → 渲染卡片 → AI 总结
```
...
AI 意图识别：这是查询玩家数量的请求
  ↓
执行 player_count 技能：查询服务器
  ↓
返回数据：{ online: 25, max: 100 }
  ↓
显示技能卡片 + AI 消息："服务器当前有 25 名玩家在线喵~"
```
...
**RAG（Retrieval-Augmented Generation，检索增强生成）** 是一种结合信息检索和文本生成的 AI 技术。它的核心思想是：在 AI 回答问题之前，先从知识库中检索相关信息，然后将这些信息作为上下文提供给 AI，让 AI 基于检索到的内容生成更准确、更专业的回答。

#### RAG 工作流程

```
用户提问 → 检索知识库 → 提取相关片段 → 构建提示词 → AI 生成回答
    ↓           ↓              ↓             ↓            ↓
 "如何加    从知识库中     找到最相关    将问题 + 知    AI 基于知
  入服务     搜索相关     的 3 个知识    识片段组合    识生成专
  器？"      内容        点          成提示词      业回答
```
...
知识库采用 Markdown 文件格式，存储在 `api/ai/knowledge_base.md` 文件中，按章节自动分块：
...
1. **读取并分割知识库**
   ```php
   $content = file_get_contents('knowledge_base.md');
   // 按章节分割（## 标题）
...
**优势：**
- ✅ 结构化存储，便于维护（Markdown 格式）
- ✅ 支持分类标签（按章节自动分块）
- ✅ 动态调整检索条数，节省 Token
- ✅ 支持子章节检索（### 子标题）
- ✅ 文件不存在时使用默认硬编码数据（容错机制）
...
└─────────────────────────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────────┐
│  1. 意图识别（api/ai/api.php）                             │
│     - AI 分析："这是查询玩家数量的请求"                     │
│     - 返回：{ skill: "player_count" }                     │
└─────────────────────────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────────┐
│  2. 执行技能（api/ai/skills.php）                          │
│     - 调用 mcstatus.php 查询服务器                        │
│     - 获得：{ online: 25, max: 100 }                      │
└─────────────────────────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────────┐
│  3. 构建回复                                               │
│     - 技能数据：{ online: 25, max: 100 }                  │
│     - 附加消息："服务器当前有 25 名玩家在线喵~"              │
└─────────────────────────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────────┐
│  4. 渲染展示（kefu.html）                                  │
│     - 显示技能卡片（玩家数量统计图）                        │
│     - 显示 AI 消息："服务器当前有 25 名玩家在线喵~"           │
└─────────────────────────────────────────────────────────┘
```

这样就能使原本只会对话的大语言模型转变为具备操作能力的**AI 智能体**。

### 文件结构

```
api/ai/
├── api.php              # AI 客服 API 接口（RAG+ 意图识别）
├── skills.php           # 技能引擎（服务器状态、玩家查询等）
├── knowledge_base.md    # 知识库（Markdown 格式，按章节自动分块）
└── prompt.md            # AI 提示词（猫娘角色设定）

pages/ai/
└── kefu.html            # 客服对话界面

components/
└── ai-assistant.html    # AI 客服按钮组件

css/
└── ai-assistant.css     # AI 客服按钮样式

js/
└── ai-assistant.js      # AI 客服按钮功能
```

### 动态读取机制

**知识库读取**：
- 从 `knowledge_base.md` 读取内容
- 按 `##` 和 `###` 标题自动分块
- 文件不存在时使用硬编码默认数据
...
注：此项功能需要用到[MCSManager](https://www.mcsmanager.com/)的CPU、内存API接口与[Minecraft-Server-Status](https://github.com/GamerNoTitle/Minecraft-Server-Status)的玩家人数API接口实现。不过关于minecraft-server-status，你可以直接使用我自行搭建的公共API使用：[http://mcstatus.goldenapplepie.xyz/status/](http://mcstatus.goldenapplepie.xyz/status/)。
