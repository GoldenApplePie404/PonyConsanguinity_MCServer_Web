...
- **pages/profile.html** - 个人中心，管理个人信息。如果你是管理员，你可以看到一个管理员面板，包括一些第三方页面的跳转链接（如[MCSManager](https://www.mcsmanager.com/)、[QQ机器人管理](https://koishi.chat/zh-CN/)等）
...
- **pages/ai/kefu.html** - 万驹同源的AI客服页面
...
知识库是客服系统用于存储和检索信息的重要组件。
知识库采用 Markdown 格式，按主题分块，存储在 `api/ai/knowledge_base.md` 文件中。

##### 4. 提示词模板

提示词是客服系统的人物设定、回答模版。你可以在 `api/ai/prompt.md` 中自定义。
...
**核心**：我们给 DeepSeek 写了一份详细的"说明书"。在`api\ai\api.php`中，我们定义了一个提示词`$skillDetectionPrompt = <<<PROMPT`，用于指导 DeepSeek 模型如何识别用户的问题。如果DeepSeek模型能判断出用户的意图以及相对应的技能，则会让其“回答”一段json内容，最后，我们只需要从返回的结果中提取json格式的意图数据即可。接下来就可以根据json的内容判断是否调用相关工具。这样就能使原本只会对话的大语言模型转变为具备操作能力的**AI 智能体**。

##### 执行流程图

```mermaid
graph LR
    A[用户发送消息] --> B[1. 前端: sendMessage]

    B --> C[2. 意图检测: detectSkill]
    ...
    D -->|Yes| E[3. 技能执行: executeSkill]
    D -->|No| F[4. 普通对话]

    E --> G[5. 后端技能引擎: SkillEngine]
    G --> H[6. 具体技能函数]
    H --> I[7. 查询 Minecraft 服务器]
    I --> J[8. 返回结果并渲染]
```
