...
- **tools/admin-hub.html** - 统一管理后台，涵盖仪表盘、服务器管理、数据库管理、订单更新、公告管理、通知发送、数据管理、图库管理、用户管理（含角色升降级）、投票管理、聊天监控、问卷管理、商城管理、AI 配置、MCP 控制台、QQ 机器人等功能面板
...
- **pages/ai/kefu.html** - 万驹同源 AI 客服页面，通过自然语言与 AI 交互管理服务器
...
管理员后台提供了一个统一的管理界面，涵盖了用户管理、公告管理、图库管理、通知发送、问卷管理、数据库查询、MCP 控制台、AI 配置等功能。管理员可以通过访问 `tools/admin-hub.html` 页面进入后台。
...
在V5.8.0版本中，新增了MCP控制台相关功能：

- **MCP 控制台**：管理员后台内置的 MCP 控制面板，支持直接调用 MCP 工具、查看执行结果，用于服务器实例管理和自动化运维。可通过侧栏「MCP 控制台」进入，左侧列出可用工具，右侧填写参数并执行，结果实时展示
- **AI 管理**：通过自然语言与 AI 客服交互，管理员可以操控服务器面板（如启动/关闭服务器、为服务器发送指令等），还能发布公告、发送通知等。AI 客服页面位于 `pages/ai/kefu.html`，API Key 和模型参数可在管理员后台的「AI 配置」面板中设置（支持自定义 API 端点、温度、Max Token 等）

##### 什么是MCP？

全称Model Context Protocol (模型上下文协议)。MCP 旨在为AI模型（特别是大型语言模型）提供一种标准化的方式来访问外部数据源和服务，从而扩展模型的能力，使其能够处理更复杂的任务。

本项目专门针对MCSManager打造了一套MCP体系，用于管理MCSManager的服务器实例。通过MCP，AI模型可以直接调用MCSManager的API来执行服务器管理任务，如启动/停止服务器、发送控制台命令、查询服务器状态等。这使得AI客服系统不仅能够进行自然语言对话，还能实际操作服务器，实现智能化的服务器管理和运维。
...
define('BOOKMARKS_FILE', dirname(__DIR__) . '/data/bookmarks.php');
define('QUESTIONNAIRES_FILE', dirname(__DIR__) . '/data/questionnaires.json'); // v5.8 新增
...
// ==================== EYPA OAuth 配置 (v5.8) ====================
define('EYPA_API_ENDPOINT', 'https://eqmemory.cn/wp-json/eu-connect/v1/user-profile');
```
