...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Version-5.5-blue?style=for-the-badge" alt="Version">
...
配置文件：`api/aifadian/config.php`

```php
return array(
    // 开发者凭证
    'user_id' => 'your-user-id', // 从爱发电开发者后台获取
    'api_token' => 'your-api-token', // 从爱发电开发者后台获取
    
    // 订单更新模式
    // 可选值: 'API' (纯API模式), 'webhook' (仅webhook模式), 'all' (webhook为主，API为辅)
    'order_update_mode' => 'all',
    
    // 自动定时任务配置
    'auto_cron' => array(
        'enabled' => true,          // 是否启用自动定时任务
        'interval' => 120,          // 执行间隔（秒），120秒=2分钟
        'max_execution_time' => 300, // 单次执行最长时间（秒）
        'log_level' => 'info'        // 日志级别：debug, info, error
    ),
    
    // 方案/商品ID
    'plan_ids' => array(
        'golden_ticket' => 'your-golden-ticket-plan-id', // 黄金券充值商品ID
        // 其他商品ID...
    ),
    
    // Webhook配置
    'webhook' => array(
        'verify_sign' => true // 是否验证签名
    )
);
```
