...
// 确保 Font Awesome 样式已引入（导航图标等依赖）
function ensureFontAwesome(base) {
    if (document.querySelector('link[href*="font-awesome"]')) return;
    const link = document.createElement('link');
    link.rel = 'stylesheet';
    link.href = base + 'css/font-awesome/all.min.css';
...
const components = {
    ...
    navbar: {
        ...
        callback: () => {
            // 确保 Font Awesome 已引入（导航图标依赖，覆盖未引入的页面）
            ensureFontAwesome(basePath);
