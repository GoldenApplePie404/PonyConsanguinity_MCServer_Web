...
function checkLoginStatus() {
    ...
    if (loginBtn && window.isLoggedIn && window.currentUser) {
        loginBtn.innerHTML = '<i class="fa-solid fa-circle-user"></i>' + window.currentUser.username;
...
// 复制服务器地址（支持传参，不传默认插件服地址）
function copyServerAddress(address, btnId) {
    const serverAddress = address || 'mc.eqmemory.cn';
    const copyBtn = document.getElementById(btnId || 'copyBtn');
    if (!copyBtn) {
